...
#### 채권
- (발행자 관점 : 재무상태표에 회사채 반영) 만기에 100,000원을 주고 이자율이 10%, 3년짜리 회사채를 발행할 경우, 실제로 그 회사채를 사는 사람은 102,531원을 주고 살 수도 있고(프리미엄 발행), 97,556원을 주고 살 수도 있음(디스카운트 발행) 이때 프리미엄 발행은 이자를 덜 줘도 된다는 뜻으로, 102,531원을 주고 산 경우 이자율 즉 수익률이 9%만 되어도 된다는 뜻. 이때는 대변 차변에 일단 102,531원을 쓰고(차변은 부채에), 첫해에는 이자를 9,228원만 주면 되는데 실제로는 10,000원을 주니 일단 부채에서 772원을 빼고(그러면 부채는 101,759원) RE에서 나머지 9,228원을 빼면 됨. 그 다음 해에는 원래는 101,759원의 9%인 9,158원만 주면 되는데 실제로는 10,000원을 주니 차액인 842원을 부채에서 빼고(남은 부채 100,917원) 9158원은 RE에서 빼면 됨. 그리고 마지막 해에는 100,917의 9%에 원금까지 주면 되는데 다 합치면 딱 110,000원 나옴. 그리고 (유효이자율) 발행비용은 유효이자에 반영해서 천천히 비용처리됨.


- (GAAP - 소유자 관점) 셋으로 나눔. Held for Trading(HFT), Available for Sale(AFS), Held to Maturity(HTM). 여기서 HTM은 발행자처럼 회계처리가 간단. 발행자와 회계처리 원리가 같음(자산과 부채에 반영). HFT는 판매가능성이 있으므로 공정가치를 평가하고, 당기손익에 반영(I/S)하고 RE에 반영. AFS도 판매가치가 있으니 공정가치를 평가하지만, 자본의 AOCI에 반영(아직 손익이 아니므로 손익계산서에도 반영 안함). 나중에 팔 때 RE로 이동.

- (cf, GAAP - 주식, 파생상품을 소유할 경우) 주식은 만기가 없음. 그래서 주식은 공정가치를 평가하고, HFT, AFS 두개만 있음. 나머지는 채권 소유자의 경우와 동일. 파생상품은 무조건 공정가치를 평가한 뒤, Hedge 목적은 ACOI, Investment 목적은 RE에 반영.

- (cf, IFRS의 소유자 관점) 용어만 좀 다름. HFT는 공정가치 당기손익인식 금융자산(Fair Value Through Profit & Loss, FVPL)이라 부름. AFS는 공정가치 기타포괄손익 인식 금융자산(Fair Value through Other Comprehensive Income, FVOCI)이라 부름. HTM는 상각후원가금융자산(Amortized Cost, AC)


#### 리스
- (리스 제공자(Lessor) 측면에서 금융리스와 운영리스) 리스의 형태이나 실제로는 할부구매와 다를 바 없는 것이 금융리스(Financial Lease). 소유권 없이 사용료를 다달이 납부하는 형태가 운영리스(Operating Lease). 이 두가지는 크게는 사용연한의 75% 이상 사용하는지 등의 기준으로 나뉨. 금융리스는 사채와 같은 회계처리방식을 따름. 운영리스는 별도. 예를 들어 비행기 계약을 하면, 비행기를 내 자산으로 잡고, 매년 감가상각하고, 리스수익이 당기손익 및 RE에 쌓임.

- (리스 이용자(Lessee) 측면에서 금융리스와 운영리스) 리스 이용자(Lessee)는 금융리스, 운영리스 모두 기존의 금융리스처럼 사채와 같은 회계처리방식을 따름. 자산에는 사용권자산(Right of Use, ROU)라고 뜨고 부채에는 1년 내에 갚는건 유동성리스부채, 1년 넘어서 갚는건 비유동성리스부채로 잡힘.

### 자본(Equity)
- (자본의 조직도) Authorized Share(발행주식총수) 아래에 Issued Shares(발행주식), Unissued Shares가 있음. Issued Shares에는 Outstanding Shares(유통주식)와 Treasury Shares(자사주 혹은 자기주식수)가 있음. 이는 GAAP에서는 정관에서 확인할 수 있고, 한국(IFRS)의 경우 주식의 총 수 칸에 있음. 재무상태표의 자본항목에는 크게 자본금(Capital Stock)+자본잉여금(Additional Paid in Capital)인 Contributed Capital과 이익잉여금이 나뉨.

- (무상증자와 유상증자) 무상증자는 CC(Contributed Capital)에서 APIC(자본잉여금)를 줄이고 CS(자본금)를 늘이는 것. 즉 자본 내에서 변동은 없음. 반면 유상증자는 APIC와 CS를 모두 늘이는 것. 즉, CC가 늘어나는 것.
...
- (개괄) 재무상태표를 기준으로 
1) 자산의 Account Receivable, Inventory, 부채의 AP의 변화는 OCF. 
2) PPE, Intangible Asset의 변화는 ICF, 
3) Equity, Borrowing Payable(금융부채)의 변화는 FCF. 
예외 1) Loan의 경우 빌려준 돈은 (ICF), 빌린 돈은 FCF로 처리.
 
        Investor        Investee
Debt    (ICF)           FCF
Equity  (ICF)           FCF

예외 2) 그런데 예외 1에서, 주식을 HFT(Held For Trading)로 가지고 있는 경우 (ICF)가 아니라 (OCF)로 봄. HFT로 기재했다는것 자체가 그것이 영업활동의 일환이라고 간주하는 것.
예외 3) 자사주 매입은 (ICF)로 보지 않음. 어떤 CashFlow를 기대하지 않기 떄문. (FCF)로 봄. 배당을 주는 것과 비슷하게 보는 것
예외 4) debt에 대한 이자, equity에 대한 배당은 OCF로 봄(GAAP). 다만 그 중에서 주식 투자자의 배당은 FCF로 봄.
...
- (채권의 종류, 국채/지방채) 국채(Soverign Bond)와 지방채(Municipal Bond)로 나뉨. 지방채는 다시 특정 프로젝트를 위해 발행한 Revenue Bond(해당 프로젝트의 수익으로 이자와 원금 지급)와 지방세로 이자와 원금을 지급하는 General Obligation Bond로 나뉨.

- (채권의 종류, Domestic/Foreign/Euro Bond) Domestic Bond는 자국민이 자국화폐로 표시한 뒤 발행한 채권을 우리나라에서 거래하는 것. Foreign Bond 는 외국민이 자국화폐로 표시한 뒤 발행한 채권을 우리나라에서 거래하는 것. Euro Bond는 외국인이 외국화폐로 표시한 뒤 발행한 채권을 우리나라에서 거래하는 것.
